...
**Architecture:** 既存の単一 fish 関数を `conf.d/gfetchprune.fish` に切り出し、責務ごとの `__gfp_*` ヘルパへ分解する。中核は「分類関数 `__gfp_classify`(各 worktree/branch をカテゴリ付きの行に変換)」で、dry-run は分類結果を整形表示し、`--execute` は分類結果を安全に削除実行する。検証は使い捨ての fixture git リポジトリに対して行う。

**Tech Stack:** fish shell, git(worktree/for-each-ref/merge-base)。テストは fish スクリプト + 一時 git リポジトリ。

## Global Constraints

- 対象シェル: fish。実装ファイルは `~/dotfiles/.config/fish/conf.d/gfetchprune.fish`(fish が `conf.d/*.fish` を自動 source)。
...
- Create: `~/dotfiles/.config/fish/conf.d/gfetchprune.fish` — `gfetchprune` 本体と `__gfp_*` ヘルパ群。
- Modify: `~/dotfiles/.config/fish/config.fish:85-237` — 旧 `gfetchprune` 関数を削除(`conf.d` へ移設)。
...
**Files:**
- Create: `~/dotfiles/.config/fish/conf.d/gfetchprune.fish`
...
function gfp_make_fixture
    ...
    __gfp_fx_branch "$work" feat/merged-pushed merge push
    # merged+gone: マージ後 origin ブランチ削除→gone
    __gfp_fx_branch "$work" feat/merged-gone merge push gone
    # gone-unmerged: 未マージで push→origin 削除→gone(固有コミットあり)
    __gfp_fx_branch "$work" feat/gone-unmerged nomerge push gone
    # unmerged+pushed: 未マージ・upstream あり
    __gfp_fx_branch "$work" feat/unmerged-pushed nomerge push
    # local-merged: マージ済み・upstream なし
    __gfp_fx_branch "$work" feat/local-merged merge nopush
    # merged-dirty: マージ済み+push(worktree を tracked dirty にする)
    __gfp_fx_branch "$work" feat/merged-dirty merge push
    # merged-untracked: マージ済み+push(worktree に untracked を置く)
    __gfp_fx_branch "$work" feat/merged-untracked merge push
    # stg: マージ済み+push(ハードコード保護)
    __gfp_fx_branch "$work" stg merge push
    # review 用: マージ済み+push(パス保護の検証に使う)
    __gfp_fx_branch "$work" feat/review-slot merge push

    git -C "$work" fetch origin --prune --quiet

    # 現在の HEAD を default 以外にする(-d/-D フォールバックと skip-current の検証)
    git -C "$work" checkout --quiet feat/unmerged-pushed
...
function __gfp_fx_branch --argument-names work name mode push gone
...
```fish
#!/usr/bin/env fish
source (status dirname)/../../.config/fish/conf.d/gfetchprune.fish
...
`~/dotfiles/.config/fish/conf.d/gfetchprune.fish`:
...
- [ ] **Step 4: 旧 gfetchprune を config.fish から削除**
...
```bash
git add .config/fish/conf.d/gfetchprune.fish .config/fish/config.fish test/gfetchprune/
...
**Files:**
- Modify: `~/dotfiles/.config/fish/conf.d/gfetchprune.fish`
...
```bash
git add .config/fish/conf.d/gfetchprune.fish test/gfetchprune/run.fish
...
```fish
set -l root (dirname (dirname "$GFP_WORK"))
...
function gfp_cat_of --argument-names rows br
    for r in $rows
        set -l p (string split \t -- $r)
        test "$p[2]" = "$br"; and echo $p[1]; and return 0
    end
end
gfp_assert "merged-pushed は delete-wt" test (gfp_cat_of "$rows" feat/merged-pushed) = delete-wt
gfp_assert "merged-gone は delete-wt" test (gfp_cat_of "$rows" feat/merged-gone) = delete-wt
gfp_assert "gone-unmerged は confirm-gone" test (gfp_cat_of "$rows" feat/gone-unmerged) = confirm-gone
gfp_assert "unmerged-pushed は keep-unmerged" test (gfp_cat_of "$rows" feat/unmerged-pushed) = keep-unmerged
gfp_assert "local-merged は protect-nopush" test (gfp_cat_of "$rows" feat/local-merged) = protect-nopush
gfp_assert "stg は protect-branch" test (gfp_cat_of "$rows" stg) = protect-branch
gfp_assert "merged-dirty は skip-dirty" test (gfp_cat_of "$rows" feat/merged-dirty) = skip-dirty
gfp_assert "merged-untracked は confirm-untracked" test (gfp_cat_of "$rows" feat/merged-untracked) = confirm-untracked
gfp_assert "review-slot は protect-path" test (gfp_cat_of "$rows" feat/review-slot) = protect-path
```
...
function __gfp_render
    set -l rows $argv
    function __gfp_section --argument-names rows title cat
        set -l hits
        for r in $rows
    ...
    __gfp_section "$rows" "削除予定 worktree(merged+push済み)" delete-wt
    __gfp_section "$rows" "削除予定 branch(worktree なし)" delete-branch
    __gfp_section "$rows" "要確認: gone-only(リモート削除済み・未マージ固有コミットあり) [--include-gone-unmerged]" confirm-gone
    __gfp_section "$rows" "要確認: untracked のみ dirty [--include-untracked-dirty]" confirm-untracked
    __gfp_section "$rows" "保護(push 無の merged)" protect-nopush
    __gfp_section "$rows" "スキップ: 保護ブランチ" protect-branch
    __gfp_section "$rows" "スキップ: 保護パス" protect-path
    __gfp_section "$rows" "スキップ: dirty(作業中)" skip-dirty
    __gfp_section "$rows" "スキップ: カレント worktree" skip-current
    __gfp_section "$rows" "detached(手動確認推奨)" detached
    __gfp_section "$rows" "残す: 未マージ" keep-unmerged
...
- [ ] **Step 1: `__gfp_emit_decision` の再走査を除去**

`__gfp_classify` の pass1/pass2 から `__gfp_emit_decision` を呼ぶ箇所で、該当ブランチの up/track を呼び出し側で引いて渡すよう変更する。`__gfp_emit_decision` のシグネチャを `--argument-names branch path merge_target kind up track` に拡張し、関数冒頭の `for m in (__gfp_branch_meta) … end` ループを削除する。classify 側は pass1/pass2 で既に展開済みの `$meta_branch`/`$meta_up`/`$meta_track` から該当インデックスの値を引いて渡す:

```fish
# classify 内のヘルパ: branch から up/track を引く
function __gfp_lookup --argument-names branch
    for i in (seq (count $meta_branch))
        if test "$meta_branch[$i]" = "$branch"
            printf '%s\t%s\n' "$meta_up[$i]" "$meta_track[$i]"
            return
        end
    end
    printf '\t\n'
...
pass1/pass2 の `__gfp_emit_decision $branch $path $merge_target wt` 呼び出しを次に変更:

```fish
        set -l ut (string split \t -- (__gfp_lookup $branch))
        __gfp_emit_decision $branch $path $merge_target wt $ut[1] $ut[2]
```

`__gfp_emit_decision` 冒頭を次に変更:
...
```bash
git add .config/fish/conf.d/gfetchprune.fish
...
**Files:**
- Modify: `~/dotfiles/.config/fish/conf.d/gfetchprune.fish`(docstring/コメント)

**Interfaces:** なし(仕上げ)。

- [ ] **Step 1: ファイル冒頭に docstring コメントを追加**

`~/dotfiles/.config/fish/conf.d/gfetchprune.fish` の先頭に:
...
- [ ] **Step 2: 新しい fish でロードされることを確認**

Run: `fish -c 'type -t gfetchprune'`
Expected: `function`(conf.d 自動 source で読み込まれる)。読まれない場合のみ、`config.fish` 末尾に `source ~/.config/fish/conf.d/gfetchprune.fish` を追加する。
